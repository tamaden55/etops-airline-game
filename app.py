...
def calculate_game_score(
    etops_compliant, co2_per_passenger, capacity_utilization, aircraft_sdg_score
):
    """
...
def display_achievement_banner(title_data, score):
    ...
    with col2:
        if title_data["color"] == "success":
            st.success(
                f"## {title_data['badge']} {title_data['title']} {title_data['badge']}\n### スコア: {score}/100\n{title_data['message']}"
            )
        elif title_data["color"] == "warning":
            st.warning(
                f"## {title_data['badge']} {title_data['title']} {title_data['badge']}\n### スコア: {score}/100\n{title_data['message']}"
...
def calculate_etops_requirement(dep_coord, arr_coord, airports_df):
    ...
    for i in range(21):  # 21 points including start and end
        ...
        max_distance_to_nearest_airport = max(
            max_distance_to_nearest_airport, min_distance
        )
...
def create_etops_map(
    dep_coord, arr_coord, dep_name, arr_name, aircraft_etops, etops_required_min
):
...
# --- Page Config ---
st.set_page_config(page_title="ETOPS Airline Strategy", page_icon="✈️", layout="wide")
...
game_mode = st.sidebar.selectbox(
    ...
    format_func=lambda x: (
        "ルート計画モード" if x == "route_planning" else "チャレンジモード"
    ),
)
...
map_type = st.sidebar.selectbox(
    ...
    ["folium", "plotly"],
    format_func=lambda x: (
        "詳細地図 (Folium)" if x == "folium" else "シンプル地図 (Plotly)"
    ),
)
...
if game_mode == "challenge_10_routes":
    ...
    with col2:
        if st.button("🎲 新しいチャレンジを開始"):
            st.session_state.challenge_routes = generate_challenge_routes(
                10, difficulty
            )
    ...
    if st.session_state.challenge_routes:
        progress = sum(
            1 for r in st.session_state.challenge_routes if r["completed"]
        ) / len(st.session_state.challenge_routes)
        st.progress(progress)
        st.write(
            f"進捗: {sum(1 for r in st.session_state.challenge_routes if r['completed'])}/10 路線完了"
        )

        # Display current route
        if st.session_state.current_route_index < len(
            st.session_state.challenge_routes
        ):
            current_route = st.session_state.challenge_routes[
                st.session_state.current_route_index
            ]
            st.subheader(
                f"路線 {current_route['route_num']}: {current_route['departure']} → {current_route['arrival']}"
            )
            st.write(
                f"距離: {current_route['distance_km']:,} km | 乗客数: {current_route['passengers']}人"
            )
            ...
            aircraft = available_aircraft[
                available_aircraft["Model"] == selected_model
            ].iloc[0]
    ...
    with col1:
        constraint_type = st.selectbox(
            ...
            ["budget", "category"],
            format_func=lambda x: (
                "💰 予算制限" if x == "budget" else "✈️ 機材カテゴリ制限"
            ),
        )
    ...
    if constraint_type == "budget":
        ...
        if len(available_aircraft) == 0:
            ...
            st.info(
                f"予算 ${budget_limit}M以内で利用可能な機材: {len(available_aircraft)}機種"
            )

            # Display available aircraft with prices
            display_df = available_aircraft[
                [
                    "Model",
                    "Category",
                    "ETOPS",
    ...
    if category_filter == "All":
        ...
        available_aircraft = aircraft_df[
            aircraft_df["Category"] == category_filter
        ].copy()
...
if departure and arrival and departure != arrival and aircraft is not None:
    ...
    # Get coordinates
    dep_coord = (
        airports_df.loc[departure, "Latitude"],
        airports_df.loc[departure, "Longitude"],
    )
    arr_coord = (
        airports_df.loc[arrival, "Latitude"],
        airports_df.loc[arrival, "Longitude"],
    )
    ...
    if (
        game_mode == "challenge_10_routes"
        and st.session_state.current_route_index
        < len(st.session_state.challenge_routes)
    ):
        ...
        if st.button("この路線を完了", key="complete_route"):
            ...
            st.session_state.challenge_routes[st.session_state.current_route_index][
                "completed"
            ] = True
            st.session_state.challenge_routes[st.session_state.current_route_index][
                "score"
            ...
            if st.session_state.current_route_index >= len(
                ...
                st.success(
                    f"🎉 チャレンジ完了！ 総合スコア: {st.session_state.challenge_total_score}/1050"
                )
    ...
    if st.session_state.map_type == "folium":
        try:
            ...
            st.info(
                "🗺️ **地図の見方**: オレンジの円はETOPS範囲を示しています。青い線が飛行ルートで、全区間がETOPS範囲内にある必要があります。"
            )
        except Exception as e:
            st.warning(f"詳細地図の表示に失敗しました。シンプル地図を表示します。")
            route_map = create_route_map_plotly(
                dep_coord,
                arr_coord,
                f"{departure} ({airports_df.loc[departure, 'Name']})",
                f"{arrival} ({airports_df.loc[arrival, 'Name']})",
    ...
    with col2:
        etops_status = (
            "✅ 適合" if etops_required_min <= aircraft["ETOPS"] else "❌ 不適合"
        )
...
st.markdown("---")
st.markdown(
    "**ETOPS Airline Strategy Game** - 持続可能な航空運航を学ぶシミュレーションゲーム"
)
...
if departure and arrival and departure != arrival:
    ...
    if st.session_state.map_type == "folium":
        try:
            ...
            st.warning(
                f"詳細地図の表示に失敗しました。シンプル地図を表示します。エラー: {str(e)}"
            )
            # Fallback to plotly
            route_map = create_route_map_plotly(
                dep_coord,
    ...
    if etops_required_min <= aircraft["ETOPS"]:
        st.success(
            f"✅ この機材（ETOPS {aircraft['ETOPS']}分）でこのルートを安全に飛行できます"
        )
    else:
        st.error(
            f"❌ この機材のETOPS性能（{aircraft['ETOPS']}分）では不十分です。必要性能: {etops_required_min:.0f}分"
        )
    ...
    if not etops_compliant:
        improvements.append(
            "🎯 **ETOPS適合で+25点**: より高性能な機材(A350-900等)を選択しましょう"
        )

    if sdg_metrics["co2_per_passenger"] > 150:
        improvements.append(
            "🌱 **環境スコアアップ**: 燃費の良い機材選択で環境スコア向上"
        )
    ...
    if aircraft["SDG_Score"] < 8:
        improvements.append(
            "⭐ **機材スコアアップ**: SDGスコアの高い機材(A350-900等)がおすすめ"
        )
...
st.markdown("**ETOPS**: Extended-range Twin-engine Operational Performance Standards")
st.markdown(
    "💡 **ヒント**: 詳細地図では各空港周辺のETOPS範囲（オレンジ円）が表示されます。高得点を目指して機材・ルート・搭乗率を最適化しましょう！"
)
